...
async function loadFromDb() {
  try {
    ...
    const { medleySongs, songPreferences, blockPreferenceLog, easterEggs } = await import('@/db/schema');
    ...
    console.error('Failed to load from database, using empty state:', error);
    return null;
  }
}
...
export default async function Page() {
  ...
  const initialMedleySongs = data?.initialMedleySongs ?? [];
  const initialStarred = data?.initialStarred ?? [];
  const initialDeleted = data?.initialDeleted ?? [];
  const initialBlockPrefLog = data?.initialBlockPrefLog ?? [];
  const initialEggs = data?.initialEggs ??
    DEFAULT_EGGS.map((egg) => ({ ...egg, id: Math.random().toString(36).substring(2, 10) }));
