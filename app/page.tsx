...
import { DEFAULT_EGGS } from '@/lib/constants';
...
export default async function Page() {
  let initialMedleySongs: MedleySong[] = [];
  let initialStarred: string[] = [];
  let initialDeleted: string[] = [];
  let initialBlockPrefLog: BPL[] = [];
  let initialEggs: EasterEgg[] = DEFAULT_EGGS.map((egg) => ({ ...egg, id: Math.random().toString(36).substring(2, 10) }));
  ...
  try {
    ...
    initialStarred = prefRows.filter(r => r.preference === 'starred').map(r => r.songId);
    initialDeleted = prefRows.filter(r => r.preference === 'deleted').map(r => r.songId);

    // Reconstruct block pref log
    initialBlockPrefLog = blockLogRows.map(row => ({
    ...
    console.error('Failed to load from database, starting with empty state:', err);
  }
